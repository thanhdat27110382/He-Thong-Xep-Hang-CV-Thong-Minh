...
class EnsembleRXTModel(torch.nn.Module):
    def __init__(self, rf_model, xgb_model, transformer_model, rf_weight=0.7, xgb_weight=0.1, transformer_weight=0.2):
...
try:
    ...
    transformer_model.load_state_dict(torch.load(os.path.join(MODEL_DIR, 'transformer_model_best.pth'), weights_only=True))
    transformer_model.eval()
    logging.info("Mô hình Transformer đã được tải thành công.")
    
    rf_xgb_transformer_model = EnsembleRXTModel(rf_model, xgb_model, transformer_model, rf_weight=0.7, xgb_weight=0.1, transformer_weight=0.2).to(device)
...
def preprocess_and_extract_features(text):
    ...
    base_range = min(1000, max(300, int(text_length * 0.3)))  
    
    scan_ranges = {
        'skills': base_range * 1.5, 
    ...
    def smart_scan(content, keyword, scan_range):
        ...
        start = max(0, pos - 50)  
        end = min(len(content), start + int(scan_range)) 
    
        next_section = re.search(r'\n\s*\n', content[int(pos):int(end)])  
        if next_section:
            end = pos + next_section.start()
    
        return content[int(start):int(end)].strip()  
...
# Hàm phân tích chi tiết mức độ khớp
def analyze_match_details(cv_feats, jd_feats):

    # Phân tích kỹ năng
    cv_skills = set(cv_feats['skills'])
    jd_skills = set(jd_feats['skills'])
    matched_skills = cv_skills & jd_skills
    missing_skills = jd_skills - cv_skills
    extra_skills = cv_skills - jd_skills
    
    # Phân tích học vấn
    cv_education = set(cv_feats['education'])
    jd_education = set(jd_feats['education'])
    matched_education = cv_education & jd_education
    missing_education = jd_education - cv_education
    extra_education = cv_education - jd_education
    
    # Phân tích kinh nghiệm
    cv_experience = set(cv_feats['experience'])
    jd_experience = set(jd_feats['experience'])
    matched_experience = cv_experience & jd_experience
    missing_experience = jd_experience - cv_experience
    extra_experience = cv_experience - jd_experience
    
    # Tính tỷ lệ khớp
    skill_match_rate = (len(matched_skills) / len(jd_skills) * 100) if jd_skills else 0
    education_match_rate = (len(matched_education) / len(jd_education) * 100) if jd_education else 0
    experience_match_rate = (len(matched_experience) / len(jd_experience) * 100) if jd_experience else 0
    ...
    return {
        'skills': {
            ...
            'match_rate': round(skill_match_rate, 1),
            'cv_total': len(cv_skills),
            'jd_total': len(jd_skills)
        },
        'education': {
            'matched': sorted(list(matched_education)),
            'missing': sorted(list(missing_education)),
            'extra': sorted(list(extra_education)),
            'match_rate': round(education_match_rate, 1),
            'cv_total': len(cv_education),
            'jd_total': len(jd_education)
        },
        'experience': {
            'matched': sorted(list(matched_experience)),
            'missing': sorted(list(missing_experience)),
            'extra': sorted(list(extra_experience)),
            'match_rate': round(experience_match_rate, 1),
            'cv_total': len(cv_experience),
            'jd_total': len(jd_experience)
        }
    }
...
def predict_scores(cv_texts, jd_text, rf_xgb_transformer_model, model_st):
    try:
        ...
        # Mã hóa embeddings
        cv_embeddings = model_st.encode(cv_cleaned_texts, show_progress_bar=True, batch_size=32)  
        jd_embedding = model_st.encode([jd_cleaned])[0]  
        
        # Tính độ tương đồng cosin
        similarities = cosine_similarity(cv_embeddings, jd_embedding.reshape(1, -1)).flatten()  
        
        # Trích xuất các tính năng NER cho mỗi cặp CV-JD
        ner_features = np.array([combine_ner_features(cv_feats, jd_feats) for cv_feats in cv_feats_list])  
        
        # Kết hợp các tính năng cho tất cả CV
        jd_embeddings_tiled = np.tile(jd_embedding, (len(cv_texts), 1))  
        similarities_array = similarities.reshape(-1, 1)  
        features = np.hstack([cv_embeddings, jd_embeddings_tiled, ner_features, similarities_array])  
        
        # Dự đoán điểm số bằng mô hình ensemble
        features_tensor = torch.tensor(features, dtype=torch.float32).to(device)
        with torch.no_grad():
            scores = rf_xgb_transformer_model(features_tensor).cpu().numpy()  
...
def upload_files():
    ...
    if not jd_text:
        ...
        return render_template('index.html', error="Job Description text is required.", jd_text=jd_text)

    # Validate CV files
    cv_files = request.files.getlist('cv_files')
    if not cv_files or all(not cv_file for cv_file in cv_files):
        logging.warning("No CV files uploaded.")
        return render_template('index.html', error="At least one CV file is required.", jd_text=jd_text)
    ...
    # Trích xuất features từ JD
    jd_result = preprocess_and_extract_features(jd_text)
    jd_features = jd_result['features']
    
    if cv_texts:
        try:
            # Tiền xử lý CV lấy cv_feats_list
            cv_results = [preprocess_and_extract_features(cv_text) for cv_text in cv_texts]
            cv_feats_list = [result['features'] for result in cv_results]

            scores = predict_scores(cv_texts, jd_text, rf_xgb_transformer_model, model_st)

            match_details_list = [analyze_match_details(cv_feats, jd_features) for cv_feats in cv_feats_list]

            for cv_name, score, cv_content, cv_features, match_details in zip(cv_names, scores, cv_contents, cv_feats_list, match_details_list):
                results.append({
                    'cv_name': cv_name,
                    'score': round(score, 2),
